...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    echo "SELECT CEILING( sum(stat.size) / 1024. / 1024. ) as logs_MB FROM public.zbx_ls_dir_log() AS logs CROSS JOIN LATERAL zbx_stat_log_file(logs) AS stat;" > /etc/zabbix/zabbix_agentd.d/query/logsize.sql && \
