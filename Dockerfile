...
RUN export DEBIAN_FRONTEND=noninteractive && \
  ...
  echo "SELECT COUNT(*) FROM pg_stat_activity WHERE (backend_xmin IS NOT NULL OR backend_xid IS NOT NULL) AND current_timestamp - query_start > interval '$1' OR current_timestamp - xact_start > interval '$1';" > /etc/zabbix/zabbix_agentd.d/query/longquery.sql && \
