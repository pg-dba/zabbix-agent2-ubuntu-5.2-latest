...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    echo "SELECT COUNT(*) as result FROM pg_stat_activity WHERE (backend_xmin IS NOT NULL OR backend_xid IS NOT NULL) AND (current_timestamp - query_start > \$1 OR current_timestamp - xact_start > \$1);" > /etc/zabbix/zabbix_agentd.d/query/longquery.sql && \
    ...
    echo "SELECT CEILING( sum(stat.size) / 1024. / 1024. ) as logs_MB FROM public.zbx_ls_dir_log() AS logs CROSS JOIN LATERAL public.zbx_stat_log_file(logs) AS stat;" > /etc/zabbix/zabbix_agentd.d/query/logsize.sql && \
