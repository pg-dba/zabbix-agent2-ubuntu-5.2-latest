...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    echo 'SELECT COUNT(*) as result FROM pg_replication_slots;' > /etc/zabbix/zabbix_agent2.d/query/rslot.sql && \
    echo "SELECT COUNT(*) as result FROM pg_stat_activity WHERE state not in ('idle in transaction', 'idle in transaction (aborted)', 'idle') AND (current_timestamp - query_start > \$1 OR current_timestamp - xact_start > \$1) AND backend_type = 'client backend';" > /etc/zabbix/zabbix_agent2.d/query/longquery.sql && \
    echo "SELECT CASE setting WHEN 'off' THEN 0 WHEN 'on' THEN 1 WHEN 'always' THEN 2 END as result FROM pg_catalog.pg_settings WHERE name = 'archive_mode';" > /etc/zabbix/zabbix_agent2.d/query/archivemode.sql && \
    echo 'SELECT CASE WHEN pg_is_in_recovery() THEN pg_is_wal_replay_paused()::integer ELSE 0 END as result;' > /etc/zabbix/zabbix_agent2.d/query/rpause.sql && \
    echo "SELECT string_agg(s,'') as result FROM (SELECT s FROM (SELECT 1 as n, 0 as seqno, '<TABLE>' as s UNION SELECT 2 as n, 0 as seqno, '<TR><TH>seqno</TH><TH>name</TH><TH>setting</TH><TH>applied</TH><TH>sourcefile</TH></TR>' as s UNION SELECT 3 as n, seqno, '<TR><TD>' || seqno || '</TD><TD>' || name || '</TD><TD>' || setting || '</TD><TD>' || applied || '</TD><TD>' || sourcefile || '</TD></TR>' as s FROM pg_catalog.pg_file_settings UNION SELECT 4 as n, 0 as seqno, '</TABLE>' as s ) tmp ORDER BY n, seqno) res;" > /etc/zabbix/zabbix_agent2.d/query/pgconfig.sql && \
    echo "SELECT CEILING( sum(stat.size) / 1024. / 1024. ) as logs_MB FROM public.zbx_ls_dir_log() AS logs CROSS JOIN LATERAL public.zbx_stat_log_file(logs) AS stat;" > /etc/zabbix/zabbix_agent2.d/query/logsize.sql && \
